...
import { AlertTriangle, ArrowUpRight, Boxes, FileQuestion, Hammer, Mail, RefreshCw, ShieldAlert } from "lucide-react";
...
function DashboardContent() {
  ...
  return (
    <AdminShell>
      <AdminPageHeader eyebrow="Gallery administration" title="Good morning, gallery team" description="A live view of the catalogue and customer conversations coming from the Django admin API." actions={<button className={styles.refresh} type="button" onClick={() => void load()} disabled={loading}><RefreshCw size={15} /> {loading ? "Loading" : "Refresh"}</button>} />
      ...
      <section className={styles.metrics} aria-label="Administration summary">
        <article><Boxes size={20} /><span><small>Products</small><strong>{summary ? count(summary.products) : "-"}</strong><em>{summary ? `${summary.products.items.filter((item) => item.status === "active").length} in this view active` : "Loading"}</em></span></article>
        <article><Mail size={20} /><span><small>Contact messages</small><strong>{summary ? count(summary.messages) : "-"}</strong><em>{summary ? `${summary.messages.items.filter((item) => item.status === "new").length} new in this view` : "Loading"}</em></span></article>
        <article><Hammer size={20} /><span><small>Custom requests</small><strong>{summary ? count(summary.customRequests) : "-"}</strong><em>{summary ? `${summary.customRequests.items.filter((item) => !["accepted", "closed"].includes(item.status)).length} open in this view` : "Loading"}</em></span></article>
        <article><FileQuestion size={20} /><span><small>FAQs</small><strong>{summary ? summary.faqs.length : "-"}</strong><em>Published records in API</em></span></article>
      </section>
      <section className={styles.columns}>
        ...
        <div className={styles.panel}>
          <div className={styles.panelHeading}><div><small>Customer conversations</small><h2>Recent enquiries</h2></div><Link href="/admin/commissions">Open requests <ArrowUpRight size={15} /></Link></div>
      ...
      <section className={styles.links}><Link href="/admin/category"><Boxes size={18} /><span><strong>Catalogue structure</strong><small>Manage categories, materials and deities</small></span><ArrowUpRight size={16} /></Link><Link href="/admin/staff"><ShieldAlert size={18} /><span><strong>Staff and security</strong><small>Available to administrator-role accounts</small></span><ArrowUpRight size={16} /></Link></section>
    </AdminShell>
  );
}
...
export function AdminDashboard() {
  ...
  if (!isLoaded) return <AdminShell><AdminState reason="storage-unavailable" /></AdminShell>;
