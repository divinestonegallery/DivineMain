  ...
  return (
    <section className={styles.section}>
      <div className={`${styles.bagLayout} site-container`}>
        <div><div className={styles.sectionHeading}><h2 className="font-display">Works to discuss</h2><button type="button" onClick={() => { enquiryBag.clear(); showToast("Enquiry bag cleared."); }}>Clear bag</button></div><div className={styles.bagItems}>{items.map((item) => <article className={styles.bagItem} key={item.id}><Link className={styles.bagThumb} href={`/products/${item.slug}`}><Image src={item.image} alt={item.imageAlt} fill sizes="110px" unoptimized={isRemoteImage(item.image)} /></Link><div><h2 className="font-display"><Link href={`/products/${item.slug}`}>{item.name}</Link></h2><p>{itemSize(item)} · {item.material} · {item.finish}</p></div><button className={styles.removeButton} type="button" aria-label={`Remove ${item.name} from enquiry bag`} onClick={() => { enquiryBag.remove(item.id); showToast("Removed from your enquiry bag."); }}><Trash2 aria-hidden="true" size={17} /></button></article>)}</div></div>
