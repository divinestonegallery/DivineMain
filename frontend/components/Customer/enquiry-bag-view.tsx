...
export function EnquiryBagView({ products }: { products: CatalogItem[] }) {
  ...
  const message = encodeURIComponent(["Namaste, I would like current details and a combined quote for:", "", ...items.map((item, index) => `${index + 1}. ${item.name} (${itemSize(item)})`), "", "Please confirm availability, pricing, packing and delivery guidance."].join("\n"));

  return (
    <section className={styles.section}>
      <div className={`${styles.bagLayout} site-container`}>
        <div><div className={styles.sectionHeading}><h2 className="font-display">Works to discuss</h2><button type="button" onClick={() => { enquiryBag.clear(); showToast("Enquiry bag cleared."); }}>Clear bag</button></div><div className={styles.bagItems}>{items.map((item) => <article className={styles.bagItem} key={item.id}><Link className={styles.bagThumb} href={`/products/${item.slug}`}><ResilientImage src={item.image} alt={item.imageAlt} fill sizes="110px" unoptimized={isRemoteImage(item.image)} fallback={<span className={styles.bagImageFallback} role="img" aria-label={`${item.name} image unavailable`}>Image coming soon</span>} /></Link><div><h2 className="font-display"><Link href={`/products/${item.slug}`}>{item.name}</Link></h2><p>{itemSize(item)} · {item.material} · {item.finish}</p></div><button className={styles.removeButton} type="button" aria-label={`Remove ${item.name} from enquiry bag`} onClick={() => { enquiryBag.remove(item.id); showToast("Removed from your enquiry bag."); }}><Trash2 aria-hidden="true" size={17} /></button></article>)}</div></div>
        <aside className={styles.bagSummary}><span className={styles.statusBadge}><ShoppingBag aria-hidden="true" size={15} /> {items.length} {items.length === 1 ? "work" : "works"}</span><h2 className="font-display">Choose quotation or checkout.</h2><p>Request a considered combined quote, or check whether every selected work has the verified price, tax, stock and shipping information required for a secure order.</p><Link className={buttonClassName({ size: "lg" })} href="/checkout">Check secure checkout <ArrowRight aria-hidden="true" size={18} /></Link><a className={buttonClassName({ variant: "outline", size: "lg" })} href={`https://wa.me/919166138566?text=${message}`} target="_blank" rel="noreferrer"><Image src="/brand/whatsapp.svg" alt="" width={18} height={18} aria-hidden="true" /> Request quote on WhatsApp</a></aside>
      </div>
    </section>
  );
}
